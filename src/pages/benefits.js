...
const Benefits = () => {
  ...
  const handleReadMoreClick = (index) => {
    if(index === readMore.selectedIndex) {
      setReadMore({
        ...readMore,
        isExpanded: false,
        selectedIndex: -1
  ...
  return (
    <main>
      ...
      <div>
        {config[0].data && config[0].data.map((item, index) => {
          return (
            <div key={index}>
              <h4>{item.title}</h4>
              <div style={descriptionStyle}>
                <span>
                  ...
                  {!readMore.isExpanded && <span> ... </span>}
                </span>
                <MoreDescription isReadMoreTriggered={readMore.selectedIndex === index}>
                  {item.description.substring(400, item.description.length)}
                </MoreDescription>
              </div>
              <div style={readMoreStyle} onClick={() => handleReadMoreClick(index)}>{readMore.isExpanded && readMore.selectedIndex===index ? "READ LESS" : "READ MORE"}</div>
            </div>
          )
        })}
      </div>
